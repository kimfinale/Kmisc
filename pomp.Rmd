...
## Simulate
```{r}
sepiar %>% simulate(params=c(gamma=0.4,vol=0.3,ba=1,bp=1,
                             fa=0.3,epsilon=0.4,delta=0.2))%>% plot()

sepiar_sim <- sepiar %>% 
  simulate(nsim = 1e2, 
           params = c(vol=0.3*1e-6,gamma=0.4,
...
sepiar %>%
  mif2(
    params=c(gamma=0.4,vol=0.2*1e-6,ba=1,bp=1,fa=0.3,
             epsilon=0.4,delta=0.2),
    Np=10000,
    Nmif=100,
    partrans=parameter_trans(log=c("epsilon")),
    paramnames=c("epsilon"),
    rw.sd=rw.sd(epsilon=0.05),
    cooling.fraction.50=0.5) -> mf1
